...
function DataForm() {
    ...
    const fetchRegions = async () => {
        try {
            ...
            setRegions(response.data.map(region => ({
                value: region.id,
                label: region.name
            })));
        } catch (error) {
    ...
    const fetchRegionDistricts = async (regionId) => {
        ...
        try {
            const response = await ApiCall(`/api/v1/district/${regionId}`, "GET", null, null);
    ...
    const getAppealType = async () => {
        try {
            ...
            setAppealType(response.data.map(type => ({
                value: type.id,
                label: type.name
            })));
        } catch (error) {
    ...
    const getEducationType = async () => {
        try {
            ...
            setEducationType(response.data.map(type => ({
                value: type.id,
                label: type.name
            })));
        } catch (error) {
    ...
    const getEducationForm = async (id) => {
        try {
            ...
            setEducationForm(response.data.map(form => ({
                value: form.id,
                label: form.name
            })));
        } catch (error) {
                                                ...
                                                <div className="bg-white p-6 md:p-8 rounded-lg shadow-lg w-full max-w-4xl mx-auto text-gray-800">
                                                    ...
                                                    <form onSubmit={handleSave} className="space-y-6">
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                            ...
                                                            {/* Father's Name */}
                                                            <div>
                                                                <label className="block text-sm font-medium text-gray-700 mb-1">Sharifi</label>
                                                                <input
                                                                    type="text"
                                                                    name="fatherName"
                                                            ...
                                                            <div>
                                                                ...
                                                                <Select
                                                                    name="regionId"
                                                                    value={regions.find(option => option.value === abuturient.regionId)}
                                                            ...
                                                            <div>
                                                                ...
                                                                <Select
                                                                    name="districtId"
                                                                    value={districts.find(option => option.value === abuturient.districtId)}
                                                                    onChange={handleSelectChange}
                                                                    options={districts}
                                                                    placeholder={loadingDistricts ? "Yuklanmoqda..." : "Tuman/Shaharni tanlang"}
                                                                    isDisabled={!abuturient.regionId || loadingDistricts}
                                                                    isSearchable
                                                                    styles={customStyles}
                                                                    required
                                                                />
                                                            ...
                                                            {/* Appeal Type */}
                                                            <div>
                                                                <label className="block text-sm font-medium text-gray-700 mb-1">Ariza turi</label>
                                                                <Select
                                                                    name="appealTypeId"
                                                                    value={appealType.find(option => option.value === abuturient.appealTypeId)}
                                                                    onChange={handleSelectChange}
                                                                    options={appealType}
                                                            ...
                                                            {/* Education Type */}
                                                            <div>
                                                                <label className="block text-sm font-medium text-gray-700 mb-1">Ta'lim turi</label>
                                                                <Select
                                                                    name="educationTypeId"
                                                                    value={educationType.find(option => option.value === abuturient.educationTypeId)}
                                                                    onChange={handleSelectChange}
                                                            ...
                                                            {/* Education Form */}
                                                            <div>
                                                                <label className="block text-sm font-medium text-gray-700 mb-1">Ta'lim shakli</label>
                                                                <Select
                                                                    name="educationFormId"
                                                                    value={educationForm.find(option => option.value === abuturient.educationFormId)}
                                                                    onChange={handleSelectChange}
                                                                    options={educationForm}
                                                                    placeholder={abuturient.educationTypeId ? "Ta'lim shaklini tanlang" : "Avval ta'lim turini tanlang"}
                                                            ...
                                                            <div>
                                                                ...
                                                                <Select
                                                                    ...
                                                                    placeholder={abuturient.educationFormId ? "Yo'nalishni tanlang" : "Avval ta'lim shaklini tanlang"}
                                                ...
                                                <div className="space-y-4">
                                                    ...
                                                    {/* Region and District */}
                                                    {
                                                        ...
                                                        <div className="flex items-center space-x-4">
                                                            ...
                                                            <p className={"my-1"}><strong>Manzil:</strong> {formData?.district.region?.name || "Noma'lum"}, {formData?.district?.name || "Noma'lum"}</p>
                                                        </div>
                                                    }
                                                    ...
                                                    <div className="mt-6">
                                                        {formData?.status >= 3 ? (
                                                            ...
                                                            formData?.educationField?.educationForm?.educationType.id === 2 ?
                                                                <div>
                                                                    <div className={"bg-cyan-100 p-4 m-4 rounded-lg"}>
                                                                        <h3 className="font-medium">Ijodiy imtihonni topshirish uchun siz institutga markaziy binosiga tashrif buyurishingiz so'raladi.</h3>
                                                                        <h4 className="mt-2 font-medium">Manzil: Buxoro shahri Sitorayi Mohi-Xosa MFY G'ijduvon ko'chasi 250-uy</h4>

                                                                        <div className='flex-1 min-w-[250px] border-r pr-4'>
                                                                            <div className='flex items-center gap-2 mt-3'>
                                                                                <CiPhone className='text-dark text-2xl' />
                                                                                <a className='text-dark hover:text-blue-600' href="tel:+998553099999">+998 55 309-99-99</a>
                                                                            </div>
                                                                            <div className='flex items-center gap-2 mb-4'>
                                                                                <CiPhone className='text-dark text-2xl' />
                                                                                <a className='text-dark hover:text-blue-600' href="tel:+998 55 305-55-55">+998 55 305-55-55</a>
                                                                            </div>
                                                                            <iframe
